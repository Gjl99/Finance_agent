...
from src.utils.chat_openai_wrapper import create_fixed_chat_openai
...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        llm = create_fixed_chat_openai(
            ...
            model_kwargs={"tool_choice": "auto"}  # 确保工具调用格式正确
        )
...
重要提示：调用工具时，请务必使用完整的股票代码（包含交易所前缀，例如 'sh.600000' 或 'sz.000001'），不要省略前缀，否则工具调用会失败。
