...
# 安全地移除常见的代理环境变量，避免使用不存在或非法的语法
import os
import sys

# 确保代理相关环境变量被移除（使用 pop 不会在变量不存在时抛出异常）
...
async def main():
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable step-by-step debug tracing output"
    )
    parser.add_argument(
        "--debug-level",
        type=str,
        ...
        default="basic",
        help="Controls granularity of debug tracing"
    )
    parser.add_argument(
        "--debug-no-color",
        action="store_true",
        help="Disable colored debug output"
    )
    parser.add_argument(
        "--debug-no-save",
        action="store_true",
        help="Do not persist intermediate agent states"
    )
    parser.add_argument(
        "--debug-no-perf",
        action="store_true",
        help="Skip performance summary tracking"
    )
    args = parser.parse_args()

    env_debug_flag = os.getenv("FINANCIAL_AGENT_DEBUG", os.getenv("FIN_AGENT_DEBUG", "0"))
    debug_enabled = args.debug or env_debug_flag.lower() in {"1", "true", "yes", "on"}
    level_map = {
    ...
    if debug_enabled:
        ...
        DEBUG_CONFIG['level'] = level_map.get(args.debug_level, DebugLevel.BASIC)
        DEBUG_CONFIG['colored_output'] = not args.debug_no_color
        DEBUG_CONFIG['save_intermediate_states'] = not args.debug_no_save
        DEBUG_CONFIG['track_performance'] = not args.debug_no_perf
    else:
        DEBUG_CONFIG['enabled'] = False
        DEBUG_CONFIG['log_file'] = None
    ...
    try:
        ...
        def wrap_agent(agent_func, label):
            """仅在调试模式下包装智能体以输出详细信息"""
            return debug_agent(label)(agent_func) if DEBUG_CONFIG['enabled'] else agent_func

        # 添加起始节点 - 作为并行分支的清晰起点
        workflow.add_node("start_node", lambda state: state)

        # 添加五个核心智能体节点
        workflow.add_node("fundamental_analyst", wrap_agent(fundamental_agent, "Fundamental Analyst"))  # 基本面分析智能体
        workflow.add_node("technical_analyst", wrap_agent(technical_agent, "Technical Analyst"))      # 技术分析智能体
        workflow.add_node("value_analyst", wrap_agent(value_agent, "Value Analyst"))             # 估值分析智能体
        workflow.add_node("news_analyst", wrap_agent(news_agent, "News Analyst"))               # 新闻分析智能体
        workflow.add_node("summarizer", wrap_agent(summary_agent, "Summary Agent"))              # 总结智能体
        ...
        if args.command:
            ...
            print("\n🔹 支持多种自然语言查询方式，建议加上股票代码，如：比亚迪(002594)")
            print("  • 分析嘉友国际(603871)")
            print("  • 帮我看看比亚迪(002594)这只股票怎么样")
            print("  • 我想了解一下腾讯的投资价值")
        ...
        def extract_stock_info(query):
            ...
            # 模式18: 直接包含5-6位数字股票代码（放宽边界限制）
            pattern18 = r'(\d{5,6})'
            match18 = re.search(pattern18, query)
            if match18 and not stock_code:
        ...
        if stock_code:
            ...
            if stock_code.startswith('6'):
                ...
                initial_data["stock_code"] = f"bj.{stock_code}"  # 北京证券交易所
            elif stock_code.startswith('9'):
                initial_data["stock_code"] = f"sz.{stock_code}"  # 深圳交易所（创业板注册制）
            else:
                initial_data["stock_code"] = stock_code

        if DEBUG_CONFIG['enabled']:
            init_debug_session(user_query, initial_data.get("stock_code"))
